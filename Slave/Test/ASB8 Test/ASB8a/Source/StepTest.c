...
static Handle_t Handle_0 = 0;         //!< Handle of stepper motor controller
static Handle_t Handle_1 = 0;         //!< Handle of stepper motor controller

static Handle_t Handle_SPIEnable = 0; 	//!< Handle of alternate SPI enable on SPI2
            ...
            switch (Command[0]) {
                ...
                case 'm':  {
                    if (ParaCount >= 1) {
                        ...
                        printf ("Move to relative position %d\n", Position);
                        Status = StepperMove (
                            Position+Home, Speed, Accel, Jerk, MicroSteps);
                    }
                    break;
                ...
                case 'g':  {
                    if (ParaCount >= 1) {
                        Position = atol(Parameters[0]);
                        printf ("Move to absolute position %d\n", Position);
                        Status = StepperMove (
                            Position+Home, Speed, Accel, Jerk, MicroSteps);
                    }
                    break;
                ...
                case 'r':  {
                    if (ParaCount >= 1) {
                        ...
                        Status = halStepperSetup(Handle_0, &Mode, 1);
                        if (Status == NO_ERROR) {
                            printf ("TMC26x register set to 0x%x\n", Mode);
						}
                        Status = halStepperSetup(Handle_1, &Mode, 1);
                        if (Status == NO_ERROR) {
                            printf ("TMC26x register set to 0x%x\n", Mode);
                        }
                    }
...
static Int32 SingleStep (Int32 SinIndex) {
   ...
   // Do next microstep, handle stepper errors
    Status = halStepperWrite (Handle_0, SinIndex);
    if (Status != NO_ERROR) {
        if (Status != LastError) {
            if (VerboseFlag) {
                Int32 ErrFlags = halStepperStatus(Handle_0, STP_STAT_ERRORFLAGS); 
                printf ("- ErrFlags=0x%x\n", ErrFlags);
            }
            LastError = Status;
        }
    }
	Status = halStepperWrite (Handle_1, SinIndex);
    if (Status != NO_ERROR) {
        if (Status != LastError) {
            if (VerboseFlag) {
                Int32 ErrFlags = halStepperStatus(Handle_1, STP_STAT_ERRORFLAGS); 
                printf ("- ErrFlags=0x%x\n", ErrFlags);
            }
            LastError = Status;
        }
...
int main (void) {
    ...
    Handle_0 = halStepperOpen (HAL_STEPPER_MOTOR_0);
    if (Handle_0 < 0) {
        printf ("Opening Stepper SFT failed\n");
        return (-1);
    }
    // lock the stepper bus
    if (halStepperControl(Handle_0, STP_CTRL_LOCK_BUS, TRUE) < 0)
        return (-1);

//halPortWrite(0x3207, 1);
//halPortWrite(0x3207, 0);

//    Handle_SPIEnable = halPortOpen (HAL_STEPPER3_SELECT, HAL_OPEN_WRITE);
//	halPortWrite(Handle_SPIEnable, 1);	// disbale stepper_NDL_SPIEnable

    Handle_1 = halStepperOpen (HAL_STEPPER_MOTOR_1);
    if (Handle_1 < 0) {
        printf ("Opening Stepper ELVT / Stepper CLP failed\n");
        return (-1);
    }
    // lock the stepper bus
    if (halStepperControl(Handle_1, STP_CTRL_LOCK_BUS, TRUE) < 0)
        return (-1);
    ...
    halStepperClose (Handle_0);
    halStepperClose (Handle_1);

	halPortClose (Handle_SPIEnable);
