...
static Error_t tempTimeVerifyPartition (const TempTimeParams_t *Params, UInt16 Channel);
...
Error_t tempTimeResetPartition (const TempTimeParams_t *Params, UInt16 Channel)
{
    ...
    if (Params->Handle >= 0) {               
        ...
        Status = bmResetStorageItems (Params->Handle, Params->PermDataTable, Params->NumberHeaters + 1);
        return (Status);
    }
...
static Error_t tempTimeVerifyPartition (const TempTimeParams_t *Params, UInt16 Channel)
{
    ...
    if (Params->Handle >= 0) {
        if (!bmVerifyStorageItems (Params->Handle, Params->PermDataTable, Params->NumberHeaters + 1)) {
            bmSignalEvent (Channel, E_PERSISTENTS_INVALID, 1, 0);            
            Error = tempTimeResetPartition (Params, Channel);
            if (Error < NO_ERROR) {
                return (Error);
            }
        }
 ...
 *  \oparam  NumberHeaters = Number of heating elements
 *
 *  \return  NO_ERROR or (negative) error code
 *
 ****************************************************************************/
...
Error_t tempTimeUpdate (TempTimeParams_t *Params, UInt32 TimeMs)
{
    UInt8 i;
    Error_t Error;
    UInt32 Counter = 0;
