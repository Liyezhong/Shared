...
UInt8 rfid11785LinkGetMissing (Rfid11785Stream_t *DataStream)
{
    UInt8 i;
    UInt8 Test;
...
Int8 rfid11785LinkComplete (Rfid11785Stream_t *DataStream)
{
    ...
    else if (DataStream->IrqError < NO_ERROR) {
        return -2;
    }
...
Bool rfid11785LinkParityOk (Rfid11785Stream_t *DataStream)
{
    ...
    for (i = 0; i < 8; i++) {
        ParityComp = rfid11785LinkComputeParity (DataStream->RxBuffer, Start + i, 9, 4);
        ParityRead = rfid11785LinkReadBit (DataStream->RxBuffer, Start + 36 + i);
        if (ParityComp != ParityRead) {
            return (FALSE);
        }
    }
