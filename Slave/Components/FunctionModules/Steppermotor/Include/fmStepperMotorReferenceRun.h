/****************************************************************************/
/*! \file fmStepperMotorReferenceRun.h
 * 
...
typedef enum {
    ...
    SM_RRS_FINISHED,                        //!< reference run successfully finished
...
typedef struct {
    ...
    UInt32                  MaxDistance;    //!< maximum number of half-steps during reference run
    UInt32                  Timeout;        //!< maximum duration to perform each movement in ms
    UInt32                  ReverseDist;    //!< distance for reverse move between high and low speed cycle
