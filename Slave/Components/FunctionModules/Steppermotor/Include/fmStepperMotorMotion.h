...
typedef struct
{
    ...
    Int32                   ResetPos;           //!< position value at which the actual position is reset to 0. Ignored if reset value is 0.

    Int8                    SinOffset;          //!< offset into sinus table. needed to ensure correct sinus values for new motion request.

    Bool                    StepLoss;           //!< indicates that step loss was detected
    Int32                   LostSteps;          //!< actual amount of lost steps (in half-step)
