 ...
 *      is supported.
 *      The encoder is counting each transition on both signals.
...
#define SM_ENCODER_MODE    TIM_MODE_ENCODER_3       //!< mode used when encoder is opened by hal ( see halEncoderOpen() )
    ...
    #define SM_ENCODER_COUNT_SHIFT 2                //!< used as multiplier to compute encoder resolution
...
Error_t smOpenEncoder (smEncoder_t *Encoder, UInt8 Instance, smMotor_t *Motor) {
    ...
    if(Encoder->Config.Exists) {
        ...
        if (RetCode < 0) {
            Error_t ErrCode;
            if ((ErrCode = smCloseEncoder(Encoder)) < 0 ) {
                RetCode = ErrCode;
            }
            return RetCode;
        }
 ...
 *      motor and encoder.
 *      In case of rotatory movement the encoder position too is reseted
...
// do correction for rotatory position value
    ...
    Encoder->Pos = Pos; // ATTENTION! because encoder position is used in ISR
                        // always modify the value in a single operation (or disable stepper interrupts)
