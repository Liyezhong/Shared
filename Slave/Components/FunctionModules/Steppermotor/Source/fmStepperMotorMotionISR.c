/****************************************************************************/
/*! \file fmStepperMotorMotionISR.c
 * 
 ...
 *      be served have to be determined first.
 *
 ...
 *      sufficient long, then motor position is checked against encoder position.
...
/******************************************************************************/
/*! 
 *  \brief  Stepper motor interrupt service routine
 *
...
//(2) reload the capture-compare register with the next value
//--------------------------------------------------------------------------
    // timer interval is calculated from actual speed
    ...
    // the irq-flag will be cleared inside the 'halCapComWrite' function
        ...
        if ((HALStatus = halCapComWrite(smHandleTimer, Instance, Motion->CCR.LastValue))< 0) {
            if (NO_ERROR == Motion->HALStatus) {
                Motion->HALStatus = HALStatus;
            }
        }
    #else
        halCapComWrite_Simplified(smTimerNo, Instance, Motion->CCR.LastValue);
    #endif
#endif /*SIMULATION*/
...
// take care of overflow for rotatory movement. correct motor, encoder and last encoder position accordingly
            if (0 != ResetPosition) {
                ...
                if (LastPos < EncPos) {                     // encoder just crossed the reset position in neg. counting direction
                ...
                else {                                      // encoder just crossed the reset position in pos. counting direction
                ...
                if (Overflow) {                             // only one of motor or encoder crossed the reset position
                    ...
                    else {                                  // encoder crossed reset position
...
//--------------------------------------------------------------------------
            if (  (0 == ResetPosition)                          // check always for linear movement
                ||(Motion->Phase > PH_4_VEL_CONST)) {           // check only during deceleration for rotatory movement
...
//--------------------------------------------------------------------------
            ...
            if (Motion->dt > ISR_ENCODER_TIME_LIMIT) {          // we can only check step difference if there is enough time to read position info from encoder in main loop
                if (MS_STOP != Motion->State) {                 // not yet terminated because of step difference or target position reached
                            ...
                            if (StepDiff >= 8) {                // a step difference of 8 half-steps means one full commutation
                                ...
                                if (PH_8_END == Motion->Phase) {
                                    if (MS_POSITION == Motion->State) {
                                        if (Motion->s >= Param->Ph[Motion->Phase].ds) { // we should have already reached the target
...
#ifndef SIMULATION
    #ifdef STEPPER_ISR_TIME_OPTIMIZATION_OFF
        HALStatus = halStepperWrite(Motor->Handle, Motion->SinIndex);
    #else
        HALStatus = halStepperWrite_Simplified(Instance, Motion->SinIndex);
    #endif
        if (HALStatus < 0) {
            if (NO_ERROR == Motion->HALStatus) {
                Motion->HALStatus = HALStatus;
...
// stop movement at half-step position if HAL reported an error
        if (Motion->HALStatus < 0) {
            Motion->State = MS_STOP;
        }
                ...
                if (Motion->ActSet != Motion->NewSet) {
                    if (Param->SwitchSet.Trigger.OldPhase == Motion->Phase) {
                        if ((Param->SwitchSet.Trigger.Position < 0) || (Motion->s == Param->SwitchSet.Trigger.Position)) {
                            ...
                            if ((Param->CCWRotDir) != (Motor->LifeCycle.DirChanges.CCWRotDir)) {
                                Motor->LifeCycle.DirChanges.Count++;
                                Motor->LifeCycle.DirChanges.CCWRotDir = Param->CCWRotDir;
                            }
                        }
                    }
                }
                ...
                if (Param->Ph[Motion->Phase].dt >= 0) {
                    if (Motion->s >= Param->Ph[Motion->Phase].ds) {
                        if (PH_8_END == Motion->Phase) {                        // target position or target speed 0 is reached 
                            if (!Encoder->Config.Exists) {                      // without encoder stop immediately
                                if (MS_POSITION == Motion->State) {
                                    Motion->AtTargetPosition = TRUE;            // target position is reached
                                }
                                else {
                                ...
                                else {                                          // target speed 0 is reached
...
//--------------------------------------------------------------------------
        if (MS_STOP == Motion->State) {
#ifndef SIMULATION
            // stop motion by disabling timer CCR irq
            if ((HALStatus = halCapComControl(smHandleTimer, Motion->CCR.Unit, TIM_INTR_DISABLE)) < 0) {
                if (NO_ERROR == Motion->HALStatus) {
                    Motion->HALStatus = HALStatus;
                }
            }
