...
void smInitLimitSwitches (smLimitSwitches_t *LimitSwitches)
{
    ...
    LimitSwitches->PosCode.TmpValue = 0;
 ...
 *      sample rate time (ms).
 *
 *      If a new postion code is available but invalid, then motor is stopped
 *      immediately.
 *      If a new postion code is available and valid, then motor is stopped if
 *      actual postion code is configured to stop motor for actual direction.
 * 
 * 
 *  \iparam  Instance       = Module instance number
