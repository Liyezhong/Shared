 ...
 *      parameters to avoid reseting profile data by a modified amount of profiles.
 *
 *
 *  \b Company:
 *
...
Error_t smConfigureParam(smData_t *Data, ConfigData_Param_t *Param, UInt8 Dlc) {
    ...
    case MOT2:
        ...
        if (Data->Profiles.Count != Param->part.mot2.flag.motionProf_cnt) {
            ...
            if (NULL == Data->Profiles.Set) {
                ...
                for (Idx = 0; Idx < Data->Profiles.Count; Idx++)
    ...
    //printf("%d:%d\n", Param->index.param.index, RetCode);
...
Error_t smConfigureProfile(smProfiles_t *Profiles, UInt8 Index, ConfigData_Param_t *Param, UInt8 Dlc) {
    ...
    //printf("%d:%d\n", Param->index.profile.index, RetCode);
    return RetCode;
}
...
// check if profiles are compatible with encoder
// profiles with full-step are not (yet) supported to be used together with the encoder
    if (Data->Encoder.Config.Exists) {
        for (Idx = 0; Idx < Data->Profiles.Count; Idx++) {
            if (1 == Data->Profiles.Set[Idx].Config.MicroSteps) {
                return E_SMOT_PROFILE_ENCODER_UNFIT;
            }
        }
    }
