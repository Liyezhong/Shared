 ...
 *      enable/disable request was successful 
 * 
...
Error_t smSetEnableState(UInt16 Channel, CanMessage_t* Message) {
    ...
    smData_t *Data;
    Error_t RetCode;
    if ((RetCode = bmGetInstance(Channel)) < 0) {
        return RetCode;
    }
    Data = &smDataTable[RetCode];

    if (NULL == Message) {
        return E_PARAMETER_OUT_OF_RANGE;
    }

    if (MSG_SMOT_SET_ENABLE_DLC != Message->Length)
    ...
    if (EnableData->enable != Data->Flags.Enable) {
        if (EnableData->enable) {   // enable the stepper
            ...
            if ((RetCode = halCapComControl(smHandleTimer, Data->Motion.CCR.Unit, TIM_INTR_DISABLE)) < 0)
                SM_SIGNAL_EVENT (RetCode);
            if (MS_IDLE != Data->Motion.State) {    //if motor is active while module gets disabled
                ...
                if (SM_STATE_IDLE == Data->State) {
                    bmSignalEvent(Channel, I_SMOT_ROTATION_STOPPED, TRUE, 0);
                }
            }
            // close hal devices, this will also switch off driver stage
            if ((RetCode = smCloseDevices(Data)) < 0)
                SM_SIGNAL_EVENT (RetCode);
...
Error_t smTargetSpeed(UInt16 Channel, CanMessage_t* Message) {
    ...
    if (SM_ACK == Ack) {
        if ((RetCode = smSpeedRequest (Data->Instance, Speed, ProfileIndex)) < 0) {
            ...
            // set stepper module state accordingly, to mark that a movement to reach target speed is active
            // inside "smModuleTask" the final acknowledge message for the movement will be sent to master
...
Error_t smReqSpeed(UInt16 Channel, CanMessage_t* Message) {
    ...
    Speed = Data->Motion.Nominal.v >> 5; // speed (in half-step/s²) = velocity / ISR_MICROSTEPS_PER_HALFSTEP;
 ...
 *      module instance. The parameters influence the behavior of the
...
// configuration is only allowed if stepper module is disabled
    ...
    if (! ConfigParam->index.type.profileData) {
        //printf("[%d] ", ConfigParam->index.param.index);
        ...
        //printf("p[%d] ", ConfigParam->index.profile.no);
...
/*****************************************************************************/
/*!
 *  \brief   Send motor direction change count (life cycle data)
 *
 *      Sends total direction change count of the motor via CAN to the master.
 *
 *  \iparam  Channel         = Logical channel number
 *  \iparam  DirChangeCount  = motor's direction change count
 *  \iparam  Ack             = success/failed status
 *
 *  \return  NO_ERROR or (negative) error code
 *
 ****************************************************************************/
static Error_t bmSendDirChangeCount (UInt16 Channel, UInt32 DirChangeCount, SM_AckState_t Ack) {

    CanMessage_t Message;

    Msg_DirChangeData_t *DirChangeData = (Msg_DirChangeData_t*)Message.Data;

    Message.CanID  = MSG_SMOT_DIRCOUNT;
    Message.Length = MSG_SMOT_DIRCOUNT_DLC;

    VAL_TO_DB4 (DirChangeCount, DirChangeData->count);
    DirChangeData->ack = Ack;
...
/******************************************************************************/
/*! 
 *  \brief  Request motor direction count count (life cycle data)
 *
 *      This function is called by the CAN message dispatcher whenever the
 *      life cycle command "request direction change count" is received.
 *      A data message is always sent back to the master.
 *
 *  \iparam  Channel = Logical channel number
 *  \iparam  Message = Received CAN message
...
Error_t smReqDirChangeCount(UInt16 Channel, CanMessage_t* Message) {
    ...
    if (MSG_SMOT_DIRCOUNT_REQ_DLC != Message->Length)
        SM_SIGNAL_EVENT (E_UNEXPECTED_PARAMETERS);

    return bmSendDirChangeCount(Channel, Data->Motor.LifeCycle.DirChanges.Count, Ack);
}
