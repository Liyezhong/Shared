...
//#define SM_SIGNAL_EVENT(channel,event,ack)          \
//    {                                               \
//        bmSignalEvent (channel, event, TRUE, 0);    \
//        ack = SM_NACK;                              \
//    }                                               \
...
//*** for DEBUG purposes only : skip need to do reference run first ***
			...
			Data->Flags.dbg_sendMovementData = EnableData->dbg_sendMovementData;
...
/*****************************************************************************/
/*! 
 *  \brief   Send CAN message containing life cycle data
 *
 *      Sends the motors actual speed data on the CAN-Bus.
 ...
 *
 ****************************************************************************/
Error_t smCanMsgSendLifeCycleData(smData_t *Data)
{
    Error_t Result;
    CanMessage_t Message;
    UInt32 StepCounter, TripCounter, OperationTime;

    Message.CanID  = MSG_SMOT_LIFE_CYCLE;
    
    // 1. message, step counter
    Message.Data[0] = 0;
    StepCounter = bmGetStorageItem(Data->Memory, PARAM_REV_COUNTER, MAX_UINT32);
    StepCounter += Data->LifeTimeData.StepCounter / Data->Motor.Config.Resolution;
    bmSetMessageItem(&Message, StepCounter, 1, 4);
    Message.Length = 5;		// MSG_SMOT_LIFE_CYCLE_DLC;
    Result = canWriteMessage(Data->Channel, &Message);
    
    // 2. message, trip counter
    Message.Data[0] = 1;
    TripCounter = bmGetStorageItem(Data->Memory, PARAM_TRIP_COUNTER, MAX_UINT32);
    TripCounter += Data->LifeTimeData.TripCounter;
    bmSetMessageItem(&Message, TripCounter, 1, 4);
    Message.Length = 5;		// MSG_SMOT_LIFE_CYCLE_DLC;
    Result = canWriteMessage(Data->Channel, &Message);    
    
    // 3. message, operation time
    Message.Data[0] = 2;
    OperationTime = bmGetStorageItem(Data->Memory, PARAM_OPERATE_HOURS, MAX_UINT32);
    OperationTime += Data->LifeTimeData.OperationTime;
    bmSetMessageItem(&Message, OperationTime, 1, 4);
    Message.Length = 5;		//MSG_SMOT_LIFE_CYCLE_DLC;
    Result = canWriteMessage(Data->Channel, &Message);
...
/******************************************************************************/
/*! 
 *  \brief   Handles the receipt of the life cycle data request
 *
 *
 *  \iparam  Channel = Logical channel number
 *  \iparam  Message = Received CAN message
...
{
    Error_t RetCode = E_COMMAND_REJECTED;
    smData_t* Data = &smDataTable[bmGetInstance(Channel)];
    
    smCanMsgSendLifeCycleData(Data);
    RetCode = NO_ERROR;
