...
#define MODULE_VERSION          0x0005  //!< Version number of module (above 60000 for test versions)
...
/******************************************************************************/
/*!
 *  \brief  Check if re-initialization is needed
 *
 ...
 *
 *      Fatal controller errors does shut down the stepper controller.
 *      To reset controller errors the controller needs to be disabled and
 *      re-enabled again. This can be accomplished by disabling the stepper
 *      module and re-enabling it.
 *
 *  \iparam  Data = Pointer to module instance's data
 *
 ******************************************************************************/
void smCheckForInitNeeded(smData_t* Data) {
    ...
    if (   (E_STEPPER_TEMPERATURE == Data->Motion.HALStatus)
        || (E_STEPPER_SHORT_CIRCUIT == Data->Motion.HALStatus)
        || (E_STEPPER_UNDERVOLTAGE == Data->Motion.HALStatus)
        || (E_STEPPER_SPI_PATTERN == Data->Motion.HALStatus)
       ) {
        Data->State = SM_STATE_INIT;  // in case of an controller error the module must be disabled and re-enabled
        bmSignalEvent(Data->Channel, I_SMOT_NEED_INIT, TRUE, 0);
    }
}
...
Error_t smMovementAck (smData_t* Data, Bool Success) {
    ...
    smCheckForInitNeeded(Data);     // may change state to SM_STATE_INIT
    smCheckEncoder(Data);
...
static Error_t smModuleTask (UInt16 Instance) {
    ...
    // !!! Important, check state (which handles end of movement cmd) before checking limit switches
