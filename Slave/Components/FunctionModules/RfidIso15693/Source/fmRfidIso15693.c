...
typedef struct {
    ...
    Error_t IrqError;            //!< Stores the code of an error occuring in an interrupt handler
...
static Error_t rfid15693OptionBitsInit (InstanceData_t* Data)
{
    ...
    if ((Data->Flags & MODE_MODULE_ENABLE) != 0 && Data->ModuleState == MODULE_STATE_BUSY) {
        if (Data->InitState == STATE_UNINIT) {
            ...
            // Start the timer
            Error = rfid15693StartCompare (Data, RFID15693_INIT_TIME);
            if (Error < NO_ERROR) {
                return (Error);
            }
 ...
 * 
 *  \xparam  Data = Contains all module instance variables
 *  \iparam  Time = Time difference to the next interrupt
 *
 *  \return  NO_ERROR or (negative) error code
 *
 *****************************************************************************/
...
static Error_t rfid15693StartCompare (InstanceData_t* Data, UInt16 Time)
{
    ...
    Data->AccTime = Data->TimeMask & (Data->AccTime + Time);
    Error = halCapComWrite (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, Data->AccTime);
    if (Error < NO_ERROR) {
        return (Error);
    }
    Error = halCapComControl (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, TIM_INTR_ENABLE);
    if (Error < 0) {
        return (Error);
    }
    return (NO_ERROR);
...
static Error_t rfid15693AcquireUid (UInt16 Channel, CanMessage_t* Message)
{
    ...
    if (Message->Length == 0) {
        if ((Data->Flags & MODE_MODULE_ENABLE) == 0) {
            ...
            Error = rfid15693StartCompare (Data, RFID15693_COMPARE_TIME);
            if (Error < NO_ERROR) {
                return (Error);
            }
...
static void rfid15693InterruptHandler (UInt32 Channel, UInt32 IntrFlags)
{
    ...
    if (Data->InitState != STATE_INIT || Data->Stream.TransmitState != LINK_TXSTATE_FINISHED) {
        // Handling the compare interrupt
        Error = halCapComStatus (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, TIM_STAT_PENDING);
        if (Error < 0) {
            Data->IrqError = Error;
        }
        else if (Error > 0) {
    ...
    if (Data->IrqError < NO_ERROR) {
        Error = halCapComControl (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, TIM_INTR_DISABLE);
        if (Error < NO_ERROR) {
            Data->IrqError = Error;
        }
...
static Error_t rfid15693InitIntHandler (InstanceData_t *Data)
{
    ...
    Data->AccTime = Data->TimeMask & (Data->AccTime + RFID15693_INIT_TIME);
    Error = halCapComWrite (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, Data->AccTime);
    if (Error < NO_ERROR) {
        return (Error);
    }
 ...
 *      front end accordingly. The function imlements the high data rate mode
...
static Error_t rfid15693CompareIntHandler (InstanceData_t *Data)
{
    ...
    Data->AccTime = Data->TimeMask & (Data->AccTime + RFID15693_COMPARE_TIME);
    Error = halCapComWrite (Data->HandleTimer, RFID15693_COMPARE_CHANNEL, Data->AccTime);
    if (Error < NO_ERROR) {
        return (Error);
    }
...
Error_t rfid15693InitializeModule (UInt16 ModuleID, UInt16 Instances)
{
    ...
    for (i=0; i < Instances; i++) {
        ...
        DataTable[i].OptionBits = bmGetBoardOptions (ModuleID, i, 0x02004033);
