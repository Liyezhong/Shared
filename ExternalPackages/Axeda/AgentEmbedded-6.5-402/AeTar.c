...
static AeInt32 _AeTarOctToInt(AeUInt8 *pData, AeInt iSize);
#define AeTarOctToInt(d, s) _AeTarOctToInt(d, s)
#else
static AeInt64 _AeTarOctToInt(AeUInt8 *pData, AeInt iSize);
#define AeTarOctToInt(d, s) ((AeInt32) _AeTarOctToInt(d, s))
#define AeTarOctToInt64(d, s) _AeTarOctToInt(d, s)
#endif
...
static AeBool AeTarReadHeader(AeTar *pTar)
{
    ...
    if (AeTarOctToInt((AeUInt8 *) pHeader->chksum, sizeof(pHeader->chksum)) !=
    ...
    pTar->curFile.iSize =
#ifndef ENABLE_LARGEFILE64
        AeTarOctToInt((AeUInt8 *) pHeader->size, sizeof(pHeader->size));
#else
        AeTarOctToInt64((AeUInt8 *) pHeader->size, sizeof(pHeader->size));
#endif

    /* read mtime (octal number string) */
    pTar->curFile.iMTime = AeTarOctToInt((AeUInt8 *) pHeader->mtime, sizeof(pHeader->mtime));
...
static AeBool AeTarWriteHeader(AeTar *pTar)
{
    ...
    snprintf(pHeader->size, sizeof(pHeader->size), "%011" AE_LONGINT_FORMAT_MODIFIER "o", pTar->curFile.iSize);

    /* write mtime (octal number string) */
    snprintf(pHeader->mtime, sizeof(pHeader->mtime), "%011lo", pTar->curFile.iMTime);
    ...
    snprintf(pHeader->chksum, sizeof(pHeader->chksum), "%07lo",
