...
{
    fprintf(stdout, "Command: set tag value: device id=%" AE_INT32_FORMAT_MODIFIER "d, tag=%s, value=",
...
void AeDemoOnSetTime(AeInt32 iDeviceId, AeTimeValue *pTime, AeInt32 *piTZOffset, AeDRMSOAPCommandStatus *pStatus)
{
    ...
    strncpy(pTimeStr, ctime(&pTime->iSec), sizeof(pTimeStr));
    pTimeStr[sizeof(pTimeStr) - 1] = 0;
    pTimeStr[strlen(pTimeStr) - 1] = 0;

    fprintf(stdout, "Command: time synchronization (%s) for device: id=%" AE_INT32_FORMAT_MODIFIER "d\n",
        pTimeStr, iDeviceId);
...
{
    fprintf(stdout, "Command: %s restart for device: id=%" AE_INT32_FORMAT_MODIFIER "d\n",
