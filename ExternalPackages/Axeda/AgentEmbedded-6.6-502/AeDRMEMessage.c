...
#ifdef __GATEWAY__
static void         AeDRMEMessageSetGateway(AeDRMEMessage *pMessage, AeChar *pOwner);
#endif
...
AeDRMEMessage *AeDRMEMessageNew(AeDRMEMessageType iType, AeInt iMajorVer, AeInt iMinorVer, AeInt32 iMaxSize, AeTimeValue *pTimeStamp)
{
    ...
    snprintf(pTmp, sizeof(pTmp), "%" AE_INT32_FORMAT_MODIFIER "d", pMessage->iId);
...
#ifdef __GATEWAY__
    AeDRMEMessageSetGateway(pMessage, pOwner);
#endif
...
AeXMLElement *AeDRMEMessageCreateSOAPCommandStatus(AeChar *pCmdId, AeChar *pUserId, AeDRMSOAPCommandStatus *pStatus)
{
    ...
    snprintf(pTmp, sizeof(pTmp), "%" AE_INT32_FORMAT_MODIFIER "x", pStatus->iStatusCode);
...
AeXMLElement *AeDRMEMessageCreatePackageStatus(AeChar *pJobId, AeInt32 iStatusCode, AeInt32 iError, AeInt32 iInstr)
{
    ...
    /* status code attribute */
    snprintf(pTmp, sizeof(pTmp), "%" AE_INT32_FORMAT_MODIFIER "d", iStatusCode);
    AeXMLElementAddAttribute(pPSElement, EMESSAGE_ATTR_STATUS_CODE, pTmp);

    /* status reason attribute */
    if (iError != AeNOERROR)
    {
        snprintf(pTmp, sizeof(pTmp), "%" AE_INT32_FORMAT_MODIFIER "d", iError);
        AeXMLElementAddAttribute(pPSElement, EMESSAGE_ATTR_ERROR_CODE, pTmp);

        snprintf(pTmp, sizeof(pTmp), "%" AE_INT32_FORMAT_MODIFIER "d", iInstr);
...
static void AeDRMEMessageSetGateway(AeDRMEMessage *pMessage, AeChar *pOwner)
{
    AeDRMDevice *pDevice = (AeDRMDevice *)
        AePtrArrayGet(g_pDRMDevices, AE_DRM_MASTER_DEVICE_ID);
    if (pDevice != NULL)
    {
        AeXMLElementSetAttribute(pMessage->pDocument->pRoot, "gwmn", pDevice->pModelNumber);
        AeXMLElementSetAttribute(pMessage->pDocument->pRoot, "gwsn", pDevice->pSerialNumber);
        AeXMLElementSetAttribute(pMessage->pDocument->pRoot, "gwow", pOwner);
    }
}
#endif
