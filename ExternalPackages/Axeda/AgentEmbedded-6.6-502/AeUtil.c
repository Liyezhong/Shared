...
void AeGetISO8601Time(AeTimeValue *pTime, AeChar *pOutput, AeInt iMaxLen)
{
    ...
    pTm = gmtime(&pTime->iSec);
    if (!pTm)
        return;

    snprintf(pOutput, iMaxLen, "%d-%02d-%02dT%02d:%02d:%02d.%03" AE_INT32_FORMAT_MODIFIER "dZ",
        pTm->tm_year + 1900,
        pTm->tm_mon + 1,
        pTm->tm_mday,
        pTm->tm_hour,
...
AeBool AeURLSet(AeURL *pURL, AeChar *pString)
{
        ...
        pDelim1 < pPath)
    {
        if ((pDelim2 = strchr(pStart, ':')) &&
            pDelim2 < pDelim1)
        {
            AeSetString(&pURL->pUser, pStart, (AeInt) (pDelim2 - pStart));
            AeSetString(&pURL->pPassword, pDelim2 + 1, (AeInt) (pDelim1 - pDelim2 - 1));
        }
        else
            AeSetString(&pURL->pUser, pStart, (AeInt) (pDelim1 - pStart));
        ...
        pDelim1 < pPath)
    {
        ...
        if (pDelim2 == pDelim1 + 1)
            return AeFalse;
        AeSetString(&pURL->pHost, pStart, (AeInt) (pDelim1 - pStart));
    }
    else
    {
        if (!pURL->bSecure)
        ...
        AeSetString(&pURL->pHost, pStart, (AeInt) (pPath - pStart));
    }
