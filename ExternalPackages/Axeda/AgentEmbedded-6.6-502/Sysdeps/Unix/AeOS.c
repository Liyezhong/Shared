...
void AeGetCurrentTime(AeTimeValue *pTime)
{
    ...
    pTime->iSec = tv.tv_sec;
    pTime->iMicroSec = tv.tv_usec;
}
...
void AeGetElapsedTime(AeTimeValue *pTime)
{
    ...
    static clock_t iLastElapsedTicks = 0;
    static long iFrequency = 0;
    static AeTimeValue lastElapsedTime = { 1, 0 };
    clock_t iCurElapsedTicks, iDeltaTicks;
    ...
    if (iInitialized == 0)
    {
        ...
        iLastElapsedTicks = iCurElapsedTicks;

        iInitialized = 1;
    }

    iDeltaTicks = iCurElapsedTicks - iLastElapsedTicks;
    if (iDeltaTicks < 0)
        iDeltaTicks = iDeltaTicks << 1 >> 1;

    deltaElapsedTime.iSec = (time_t) (iDeltaTicks / iFrequency);
    deltaElapsedTime.iMicroSec = (AeInt32) ((iDeltaTicks % iFrequency) * 1000000 / iFrequency);
