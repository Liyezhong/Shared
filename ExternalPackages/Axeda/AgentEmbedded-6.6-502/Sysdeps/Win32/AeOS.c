...
void AeGetElapsedTime(AeTimeValue *pTime)
{
    ...
    static DWORD iLastElapsedMillis = 0;
    static AeTimeValue lastElapsedTime = { 1, 0 };
    DWORD iCurElapsedMillis, iDeltaMillis;
    AeTimeValue deltaElapsedTime;

    iCurElapsedMillis = GetTickCount();

    if (iInitialized == 0)
    {
        iLastElapsedMillis = iCurElapsedMillis;
        iInitialized = 1;
    }

    iDeltaMillis = iCurElapsedMillis - iLastElapsedMillis;
    deltaElapsedTime.iSec = (time_t) (iDeltaMillis / 1000);
    deltaElapsedTime.iMicroSec = (AeInt32) ((iDeltaMillis % 1000) * 1000);
    AE_TIME_VALUE_ADD(*pTime, lastElapsedTime, deltaElapsedTime);

    iLastElapsedMillis = iCurElapsedMillis;
    lastElapsedTime = *pTime;
}
...
{
    DWORD iMilliseconds = (DWORD) AE_TIME_VALUE_MILLISECONDS(*pTime);
    Sleep(iMilliseconds);
}
