...
void ActionHandler::ReceiveEvent(const DataLogging::DayEventEntry &TheEvent, const quint32 EventKey) {
    ...
    if(TheEvent.GetGUIMessageBoxOptions()!= Global::NO_BUTTON)
    {
    ...
    if((TheEvent.GetActionType()== Global::ACNTYPE_BUSY) || (TheEvent.GetActionType()== Global::ACNTYPE_IDLE) || (TheEvent.GetActionType()== Global::ACNTYPE_STOP))
    {
