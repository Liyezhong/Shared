...
const quint32 EVENT_GLOBAL_ERROR_FILE_DELETE                = EVENT_GROUP_PLATFORM_GLOBAL + 0x000F; ///< Error deleting file %1.
...
const quint32 EVENT_GLOBAL_SETTINGS_ROLLED_BACK             = EVENT_GROUP_PLATFORM_GLOBAL + 0x001B; //!< Filesystem error occured. System is rolled back to last known good configuration.
const quint32 EVENT_RECOVERED_FROM_POWER_FAIL               = EVENT_GROUP_PLATFORM_GLOBAL + 0x001C; //!< Instrument recovered from power failure. Remove all racks.
...
const quint32 EVENT_DEVICE_START_INFO                                   = EVENT_GROUP_PLATFORM_GLOBAL + 0x0229; //!< %1 %2 started. Serial Number: %3. MAC address: %4
...
const quint32 EVENT_THREAD_CREATE_AND_INITIALIZE_FAILED                     = EVENT_GROUP_SHARED_EVENTS + 0x0006; //!< Create and initialize failed
