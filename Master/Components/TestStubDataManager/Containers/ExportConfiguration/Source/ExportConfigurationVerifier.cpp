...
bool CExportConfigurationVerifier::VerifyData(CDataContainerBase* p_Configuration)
{
    ...
    if (mp_ECD != NULL) {
        ...
        if(mp_ECD->GetSourceDir().compare("") != 0) {
            ...
            if (!Directory.exists(mp_ECD->GetSourceDir())) {
                VerifiedData = false;
            }
        }
        ...
        if(mp_ECD->GetTargetDir().compare("") != 0) {
            // check the existence of directory
            if (!Directory.exists(mp_ECD->GetTargetDir())) {
                VerifiedData = false;
            }
            else {
...
/****************************************************************************/
/*!
 *  \brief  Check the files and the path of the files
 *
    ...
    if (mp_ECD != NULL) {
        ...
        foreach (QString FileName, Configuration.GetFileList()) {
            ...
            if (FileName.compare("") != 0) {
                ...
                if(!QFile::exists(mp_ECD->GetSourceDir() + QDir::separator() + FileName)) {
                    Result = false;
                }
            }
        }
        ...
        if (Configuration.GetGroupListFlag()) {
            ...
            if(Configuration.GetGroupFileName().compare("") != 0) {
                // check the existence of directory
                QDir Directory;
                if (!Directory.exists(mp_ECD->GetSourceDir() + QDir::separator() + Configuration.GetGroupFileName())) {
                    Result = false;
                }
            }
        }        
