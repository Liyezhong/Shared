...
CPressureControl::CPressureControl(const CANMessageConfiguration *p_MessageConfiguration, CANCommunicator* pCANCommunicator, CBaseModule* pParentNode) :
    ...
    m_ActiveCarbonFilterLifeTime(0),
    m_ExhaustFanLifeTime(0)
{
    // main state
...
void CPressureControl::HandleCANMsgServiceSensor(can_frame* pCANframe)
{
    ...
    if(pCANframe->can_dlc == 3)
    {
        ...
        float ActPressure = (float)GetCANMsgDataS16(pCANframe, 1) / 100;
...
void CPressureControl::SetValveLifeCycle(quint32 valve1Cycle, quint32 valve2Cycle)
{
    m_valveOperationTime[0] = valve1Cycle;
    m_valveOperationTime[1] = valve2Cycle;
}

quint32 CPressureControl::GetValveLifeCycle(quint32 ValveIndex)
{
    if(ValveIndex < VALVE_NUM)
    {
