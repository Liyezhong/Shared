...
{   
    ...
    m_ModuleInformation.SetModuleName(this->name());
    ...
    while (Iterator.hasNext()) {
        ...
        switch (Iterator.value()->GetType()) {
            ...
            case CModuleConfig::CAN_OBJ_TYPE_DIGITAL_IN_PORT:
                p_NewState = CreateInfoModule<CInfoDigitalInput>(dynamic_cast<CDigitalInput *>(Iterator.value()),
            ...
            case CModuleConfig::CAN_OBJ_TYPE_PRESSURE_CTL:
                p_NewState = CreateInfoModule<CInfoPressureControl>(dynamic_cast<CPressureControl *>(Iterator.value()),
                                                               QString("Pressure Control"), p_Active);
                break;
            case CModuleConfig::CAN_OBJ_TYPE_OTHER:
                p_NewState = CreateInfoModule<CInfoOtherModule>(dynamic_cast<COtherModule *>(Iterator.value()),
                                                           QString("Other Module"), p_Active);
            break;
            default:
                break;
        }
